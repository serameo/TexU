...
void _TexuUpDownCtrlProc_OnKeyDown(texu_wnd *wnd, texu_i32 ch, texu_i32 alt)
{
    ...
    switch (ch)
    {
        case TEXU_KEY_UP:
        case TEXU_KEY_RIGHT:
            _TexuUpDownCtrlProc_OnStep(wnd, 1);
            break;
        case TEXU_KEY_DOWN:
        case TEXU_KEY_LEFT:
            _TexuUpDownCtrlProc_OnStep(wnd, -1);
            break;
        case TEXU_KEY_NPAGE:
            _TexuUpDownCtrlProc_OnPage(wnd, 1);
            break;
        case TEXU_KEY_PPAGE:
            _TexuUpDownCtrlProc_OnPage(wnd, -1);
            break;
...
_TexuUpDownCtrlProc_OnChar(texu_wnd *wnd, texu_i32 ch, texu_i32 alt)
{
    ...
    {
        texu_wnd_send_msg(udctl->editwnd, TEXU_WM_CHAR, (texu_lparam)ch, 0);
    }
}
    ...
    while (item)
    {
        ...
        if ((hchilds + h) > height)
        {
            ...
            else
            {
                ...
                texu_wnd_move(band->childwnd, y, x + capwidth, h, w, TEXU_TRUE);
                ...
                {
                    texu_memset(unit, 0, sizeof(unit));
                    texu_memset(unit, ' ', unitwidth);
                }
#ifdef TEXU_CIO_COLOR_MONO
                        texu_cio_putstr_attr(dc, y, x + capwidth + w,
                            unit,
                            texu_cio_get_color(dc, color));
#else
                        texu_cio_draw_text(dc, y, x + capwidth + w,
                            unit, color, bgcolor,
                            texu_wnd_get_clsname(wnd),
                            texu_wnd_get_id(wnd));
#endif
        ...
        if (TEXU_RBS_HASUNIT & style && h > 1)
        {
            texu_i32 i = 1;
            texu_memset(unit, 0, sizeof(unit));
            texu_memset(unit, TEXUTEXT(' '), unitwidth);
            for (; i < h; ++i)
            {
#ifdef TEXU_CIO_COLOR_MONO
            texu_cio_putstr_attr(dc, y + i, x + capwidth + w,
                unit,
                texu_cio_get_color(dc, color));
#else
            texu_cio_draw_text(dc, y + i, x + capwidth + w,
                unit, color, bgcolor,
                texu_wnd_get_clsname(wnd),
                texu_wnd_get_id(wnd));
#endif
